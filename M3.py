...
def solve_M3(number_of_nodes, edges):
    ...
    y = [m.add_var(var_type=BINARY) for j in range(number_of_nodes-1)]
    ...
    # (9) The second way is to strengthen the constraint int(n/k) ≤ |Cj | ≤ int(n/k) + 1 by the disjunction
    # |Cj | = int(n/k) ∨ |Cj | = int(n/k) + 1. We add binary variables yj
    for j in range(number_of_nodes - 1):
        m += xsum(x[v][j] for v in range(number_of_nodes)) == y[j] + w[number_of_nodes-1] + \
